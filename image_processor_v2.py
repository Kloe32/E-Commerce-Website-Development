...
def process_one(img_path, out_file, params):
    # Import process_image INSIDE the function so it's available to subprocesses
...
class BatchImageProcessorApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Batch Image Processor v2 (Parallel/Process)")
    ...
    def process_images(self):
        ...
        def run():
            try:
                ...
                out_files = [os.path.join(output_dir, f"{os.path.splitext(os.path.basename(f))[0]}.{output_format.lower()}") for f in files]

                with ProcessPoolExecutor(max_workers=10) as executor:
                    futures = [
                        executor.submit(process_one, img_path, out_file, params)
                        for img_path, out_file in zip(files, out_files)
                    ]
                ...
                self.status_label.config(text=f"Done! Processed {total} images in {elapsed:.2f} seconds (Parallelized).")
